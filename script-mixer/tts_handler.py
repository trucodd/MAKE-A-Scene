...
def generate_tts_audio(text: str, voice_id: str = "en-US-davis", voice_style: str = "Conversational") -> str:
    ...
    # Get API key from environment (Docker sets this)
    murf_api_key = os.getenv("MURF_API_KEY")
    
    print(f"Debug - MURF_API_KEY exists: {bool(murf_api_key)}")
    print(f"Debug - MURF_API_KEY length: {len(murf_api_key) if murf_api_key else 0}")
    
    if not murf_api_key:
        print("No Murf API key found in environment")
        return None
